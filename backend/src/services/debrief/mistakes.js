/**
 * Deterministic mistake-pattern detector over session.rounds.
 * Runs before LLM debrief; results are merged into debriefData.lessonMistakes.
 */
...
/** Curriculum-aligned mistakes when the player missed the optimal choice that round. */
...
/**
 * @param {{ rounds?: object[], finalMetrics?: object }} session
 * @returns {object[]} 3–5 lesson mistakes, sorted by severity
 */
function detectMistakePatterns(session) {
  const rounds = session.rounds || [];
  const final = session.finalMetrics || {};
  const found = [];
  ...
  for (const r of rounds) {
    ...
    if (template) {
      found.push(enrichMistake(template, r));
    }
  }
